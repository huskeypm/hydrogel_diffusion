...
Point(17) = { 0, 0, -aqD, lc};
Point(18) = { bR, 0, -aqD, lc};
Point(19) = { 0, -bR, -aqD, lc};
Point(20) = { -bR, 0, -aqD, lc};
Point(21) = { 0, bR, -aqD, lc};
...
"""
    ...
    text+= "Plane Surface(2) = {2,%s};\n"%(listExtrudedCircleNames)
    text+= "Surface Loop(1) = {1,%s,6,5,3,7,4,13,out%s[0],out%s[2],out%s[3],out%s[4],out%s[5]};\n"%(lineNames,cirNum,cirNum,cirNum,cirNum,cirNum)
    text+= "Volume(%s) = {1};\n"%(cirNum)
    ...
    if num%2!=0:
        while i <num:
            ...
            while j < num:
                ...
                if 0:#(abs(x)-radius/2)**2+(abs(y)-radius/2)**2<bR**2:
                    this = False
                else:
                    centers.append((x,y))
                j+=1
...
Notes:
  ...
  Example: python nanoRealMesher.py -makem "8,1e-9,9e-9,15e-9,10e-9,10e-9,name"
